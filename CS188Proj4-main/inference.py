...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #combin = list(itertools.combinations(pos, 2))
    #obs1 = []
    #for i in combin:
    #    obs1.append(abs(i[0][0] - i[1][0]) + abs(i[0][1] - i[1][1]) + random.random()*MAX_NOISE)
    maxPossible = X_RANGE + Y_RANGE + 6
    variableDomainsDict[OBS0] = [i for i in range(maxPossible)]
    variableDomainsDict[OBS1] = [i for i in range(maxPossible)]
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        c = 1
        while c < len(vals):
             runningSum.append(vals[c] + runningSum[c-1])
             c += 1
        #print(runningSum)
        ...
        #print(ks)
        c = 0
        while c < len(runningSum):
            if sampleNumber < runningSum[c]:
                return ks[c]
            c += 1
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        dis = manhattanDistance(ghostPosition, jailPosition)
        ...
        if dis == 0:
            return 0
        PnGt = busters.getObservationProbability(noisyDistance, manhattanDistance(pacmanPosition, ghostPosition))
        return PnGt
        #raiseNotDefined() 
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            prob = self.getObservationProb(observation, gameState.getPacmanPosition(), i, self.getJailPosition())
            self.beliefs[i] = self.beliefs[i]*prob
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newPosDists = {}
        for i in self.allPositions:
            newPosDists[i] = self.getPositionDistribution(gameState, i)
        #newPosDists = [self.getPositionDistribution(gameState, i) for i in self.allPositions]
        bel = self.beliefs.copy()
        for i in self.allPositions:
            #newPosDist = self.getPositionDistribution(gameState, i)
            out = []
            #bel = self.beliefs.copy()
            for j in self.allPositions:
                #newPosDist = self.getPositionDistribution(gameState, j)
                out.append(newPosDists[j][i]*bel[j])
            self.beliefs[i] = sum(out) 
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        remand = self.numParticles%len(self.legalPositions)
        numParts = self.numParticles
        while numParts > len(self.legalPositions):
            for i in self.legalPositions:
                self.particles.append(i)
                numParts -= 1
        if remand == 0:
            return
        moveBy = len(self.legalPositions)//remand
        c = 0
        index = 0
        while c < remand:
        ...
        #stop = False
        #moveBy = 0
        #spacing  = self.numParticles
        #print(numPos)
        #print(self.numParticles)
        #while not stop:
        #     moveBy = numPos//spacing
        #     if moveBy == 0:
        #         amount += 1
        #         spacing = spacing - numPos
        #         continue
        #     stop = True
        #c = 0
        ...
        #        #    self.particles.append(pos[0])
        #        #total -= 1
        #    self.particles.append(pos[c])
        #    total -= 1 
        #    c += moveBy
        #for i in pos:
